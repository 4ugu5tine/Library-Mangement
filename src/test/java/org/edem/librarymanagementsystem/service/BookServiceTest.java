...
class BookServiceTest {
    ...
    void getBookById() throws SQLException {
        ...
        when(mockResultSet.getString("title")).thenReturn("Book One");
        when(mockResultSet.getString("author")).thenReturn("Author One");
        when(mockResultSet.getString("publisher")).thenReturn("Publisher One");
        when(mockResultSet.getInt("yearPublished")).thenReturn(2020);
        when(mockResultSet.getBoolean("isAvailable")).thenReturn(true);
        when(mockResultSet.getString("genreId")).thenReturn("Fiction");
        when(mockResultSet.getInt("copies")).thenReturn(5);
        ...
        assertEquals("Author One", book.getAuthor());
        assertEquals("Publisher One", book.getPublisher());
        ...
        Book book_exception = bookService.getBookById(1);
    }

    @Test
    void searchBooks() throws SQLException {
        when(mockPreparedStatement.executeQuery()).thenReturn(mockResultSet);
        ...
        when(mockResultSet.getInt("year_published")).thenReturn(2020).thenReturn(2021);
        ...
        LinkedList<Book> books = bookService.searchBooks("Books");
        ...
        when(mockPreparedStatement.executeQuery()).thenThrow(SQLException.class);
        LinkedList<Book> books_exception = bookService.searchBooks("Books");
    ...
    void getAllBooks() throws SQLException {
        ...
        when(mockPreparedStatement.executeQuery()).thenThrow(SQLException.class);
        LinkedList<Book> books_exception = bookService.getAllBooks();
